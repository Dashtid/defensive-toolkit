...
class OpenSearchDeployer:
    ...
    def _connect(self) -> OpenSearch:
        ...
        try:
            client = OpenSearch(
                hosts=[{
                    'host': self.os_config['host'],
                    'port': self.os_config['port']
                }],
                http_auth=(
    ...
    def deploy_rules(self, rules_dir: str, dry_run: bool = False) -> Dict:
        ...
        for sigma_file in sigma_files:
            try:
                ...
                if dry_run:
                    logger.info(f"[DRY RUN] Would deploy: {rule_data.get('title', sigma_file.name)}")
                    stats['deployed'] += 1
                else:
...
def main():
    ...
    parser.add_argument('--rules-dir', default='../../detection-rules/sigma', help='Rules directory')
