...
class TestDetectionRulesEndpoints:
    ...
    def test_list_rules_with_filters(self, auth_headers):
        ...
        response = client.get(
            "/api/v1/detection/rules?rule_type=sigma&severity=high", headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
        assert "rules" in data
    ...
    def test_create_sigma_rule_success(self, auth_headers):
        ...
        rule_data = DetectionRuleFactory.create(
            name="Test Sigma Rule", rule_type="sigma", severity="high"
        )
    ...
    def test_create_rule_missing_required_fields(self, auth_headers):
        ...
        response = client.post(
            "/api/v1/detection/rules", json=incomplete_rule, headers=auth_headers
        )
    ...
    def test_get_rule_by_id_success(self, auth_headers):
        ...
        create_response = client.post(
            "/api/v1/detection/rules", json=rule_data, headers=auth_headers
        )
        rule_id = create_response.json()["id"]

        # Get the rule
        response = client.get(f"/api/v1/detection/rules/{rule_id}", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_rule_not_found(self, auth_headers):
        ...
        response = client.get("/api/v1/detection/rules/nonexistent-id", headers=auth_headers)
    ...
    def test_update_rule_success(self, auth_headers):
        ...
        response = client.patch(
            f"/api/v1/detection/rules/{rule_id}", json=update_data, headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_delete_rule_success(self, auth_headers):
        ...
        # Delete the rule
        response = client.delete(f"/api/v1/detection/rules/{rule_id}", headers=auth_headers)
        assert response.status_code == 200

        # Verify rule is deleted
        get_response = client.get(f"/api/v1/detection/rules/{rule_id}", headers=auth_headers)
...
class TestDetectionRuleValidation:
    ...
    def test_validate_sigma_rule_success(self, auth_headers):
        ...
        rule_data = {"rule_type": "sigma", "content": "detection:\n  selection:\n    EventID: 4688"}
        response = client.post(
            "/api/v1/detection/rules/validate", json=rule_data, headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_validate_invalid_rule(self, auth_headers):
        ...
        rule_data = {"rule_type": "sigma", "content": "invalid: yaml: content:"}
        response = client.post(
            "/api/v1/detection/rules/validate", json=rule_data, headers=auth_headers
        )
        assert response.status_code == 200
...
class TestDetectionRuleBulkOperations:
    ...
    def test_bulk_create_rules_success(self, auth_headers):
        ...
        response = client.post(
            "/api/v1/detection/rules/bulk", json={"rules": rules}, headers=auth_headers
        )
    ...
    def test_bulk_delete_rules_success(self, auth_headers):
        ...
        create_responses = []
        for rule in rules:
            resp = client.post("/api/v1/detection/rules", json=rule, headers=auth_headers)
