...
class AnomalyDetector:
    ...
    def _compute_statistics(self, log_entries: List[Dict]) -> Dict:
        ...
        if stats["message_lengths"]:
            ...
            stats["message_length_stddev"] = (
                statistics.stdev(stats["message_lengths"])
                if len(stats["message_lengths"]) > 1
                else 0
            )
                    ...
                    self.anomalies.append(
                        {
                            ...
                            "details": {"process": process, "count": count, "frequency": frequency},
                        }
                    )
                    ...
                    self.anomalies.append(
                        {
                            ...
                            "details": {"ip": ip, "count": count, "frequency": frequency},
                        }
                    )
...
Examples:
  ...
  python anomaly-detector.py --detect --input logs.json --output anomalies.txt
    ...
    parser.add_argument("--create-baseline", action="store_true", help="Create baseline statistics")
    parser.add_argument("--detect", action="store_true", help="Detect anomalies")
    parser.add_argument(
        "--input", "-i", type=Path, required=True, help="Input file (parsed JSON logs)"
    )
    parser.add_argument("--baseline", "-b", type=Path, help="Baseline statistics file")
    parser.add_argument("--output", "-o", type=Path, help="Output file")
    parser.add_argument(
        "--output-format", choices=["json", "text"], default="text", help="Output format"
    )
    parser.add_argument(
        "--threshold", type=float, default=2.0, help="Anomaly threshold (std devs, default: 2.0)"
    )
    ...
    detector = AnomalyDetector(baseline_file=args.baseline, threshold_stddev=args.threshold)
