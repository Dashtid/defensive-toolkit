...
class LogParser:
    ...
    def _parse_generic(self, line: str) -> LogEntry:
        ...
        return LogEntry(
            timestamp=timestamp, source_ip=source_ip, dest_ip=dest_ip, message=line, raw=line
        )
...
Examples:
  ...
  python log-parser.py --file app.log --format json
    ...
    parser.add_argument("--file", "-f", type=Path, required=True, help="Log file to parse")
    parser.add_argument(
        "--format",
        choices=["auto", "syslog", "json", "apache", "nginx"],
        default="auto",
        help="Log format (default: auto)",
    )
    parser.add_argument("--output", "-o", type=Path, help="Output JSON file")
    parser.add_argument("--max-lines", type=int, help="Maximum lines to parse")
    parser.add_argument("--filter", type=str, help="Filter messages containing string")
