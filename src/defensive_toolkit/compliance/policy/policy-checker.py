...
class PolicyChecker:
    ...
    def _check_command(self, check: Dict) -> Dict:
        ...
        try:
            ...
            if proc.returncode != expected_return_code:
                result['status'] = 'FAIL'
                result['message'] = f'Command returned {proc.returncode}, expected {expected_return_code}'
            ...
            if expected_output:
                ...
                if match_type == 'exact':
                    if output == expected_output:
                        ...
                        result['status'] = 'FAIL'
                        result['message'] = f'Output mismatch. Expected: {expected_output}, Got: {output}'
        ...
        try:
            ...
            if (expected_status == 'running' and is_running) or \
               (expected_status == 'stopped' and not is_running):
                ...
                result['message'] = f'Service {service_name} is {actual}, expected {expected_status}'
            ...
            if self.os_type == 'windows':
                ...
                if expected_mode.startswith('0'):
                    ...
                    if actual_mode == expected_mode:
                        ...
                        result['status'] = 'FAIL'
                        result['message'] = f'Permission mismatch. Expected: {expected_mode}, Got: {actual_mode}'
                else:
                    result['status'] = 'SKIPPED'
    ...
    def _generate_text_report(self, output_file: Optional[Path] = None) -> str:
        ...
        compliance_rate = (summary['passed'] / summary['total'] * 100) if summary['total'] > 0 else 0
...
Examples:
  ...
  python policy-checker.py --policy baseline-security.yaml --verbose
    ...
    parser.add_argument('--policy', '-p', type=Path, required=True,
                       help='Policy definition file (YAML)')
