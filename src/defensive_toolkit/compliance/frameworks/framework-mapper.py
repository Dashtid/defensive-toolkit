...
class FrameworkMapper:
    ...
    def export_mapping(self, output_format: str = 'json', output_file: Optional[Path] = None) -> str:
...
Examples:
  ...
  python framework-mapper.py --export --output-format json
    ...
    parser.add_argument('--map', type=str,
                       help='Map a specific control (e.g., CIS-1, NIST-AC)')
    parser.add_argument('--overlaps', nargs='+', type=str,
                       help='Find overlaps between frameworks')
    parser.add_argument('--coverage', type=str,
                       help='Generate coverage matrix for target framework')
    parser.add_argument('--recommend', nargs='+', type=str,
                       help='Recommend implementation order for target frameworks')
    parser.add_argument('--export', action='store_true',
                       help='Export full mapping database')
    ...
    if args.map:
        ...
        if result:
            print(json.dumps(result, indent=2) if args.output_format == 'json' else
