...
class NISTChecker:
            ...
            checks.append(
                {
                    ...
                    "details": (
                        "PAM modules for failed login tracking"
                        if has_faillock
                        else "Manual configuration required"
                    ),
                    "baseline": ["moderate", "high"],
                }
            )
    ...
    def check_au_audit_accountability(self) -> Dict:
        ...
        if self.os_type == "windows":
            ...
            has_syslog = (
                self._command_exists("rsyslogd")
                or self._command_exists("syslog-ng")
                or self._command_exists("journalctl")
            )
            checks.append(
                {
                    "control": "AU-2",
    ...
    def check_cm_configuration_management(self) -> Dict:
        ...
        if self.os_type == "windows":
            ...
            has_tools = (
                self._command_exists("ansible")
                or self._command_exists("puppet")
                or self._command_exists("chef-client")
            )
            checks.append(
                {
                    "control": "CM-6",
            ...
            checks.append(
                {
                    ...
                    "details": (
                        ...
                        if pam_pwquality
                        else "Password quality configuration"
                    ),
                    "baseline": ["moderate", "high"],
                }
            )
    ...
    def check_sc_system_communications_protection(self) -> Dict:
        ...
        if self.os_type == "windows":
            ...
            is_enabled = (
                firewall_check and "State" in str(firewall_check) and "ON" in str(firewall_check)
            )
            checks.append(
                {
                    "control": "SC-7",
            ...
            has_firewall = (
                self._command_exists("iptables")
                or self._command_exists("nft")
                or self._command_exists("ufw")
            )
            checks.append(
                {
                    "control": "SC-7",
            ...
            checks.append(
                {
                    ...
                    "details": (
                        "LUKS encryption tools available"
                        if cryptsetup_exists
                        else "Encryption tools"
                    ),
                    "baseline": ["moderate", "high"],
                }
            )
            ...
            checks.append(
                {
                    ...
                    "details": (
                        "ClamAV installed" if clamav_exists else "Anti-malware solution recommended"
                    ),
                    "baseline": ["low", "moderate", "high"],
                }
            )
...
Examples:
  ...
  python nist-checker.py --impact-level high --output-format text
    ...
    parser.add_argument(
        ...
        default="moderate",
        help="FIPS 199 impact level (default: moderate)",
    )
    parser.add_argument(
        "--output-format",
        choices=["json", "text"],
