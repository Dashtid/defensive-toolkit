...
class TimelineAnalyzer:
    ...
    def detect_activity_spikes(self, window_minutes: int = 60) -> None:
        ...
        # Find spikes
        spikes = [(window, count) for window, count in window_counts.items() if count > threshold]
        ...
        for i, event1 in enumerate(self.events):
            ...
            for event2 in self.events[i + 1 :]:
                ...
                for pattern in suspicious_patterns:
                    if any(keyword in desc1 for keyword in pattern) and any(
                        keyword in desc2 for keyword in pattern
                    ):
                        correlations.append(
                            {
    ...
    def detect_off_hours_activity(self, work_start_hour: int = 8, work_end_hour: int = 18) -> None:
        ...
        """
        logger.info(
            f"[+] Detecting off-hours activity (work hours: {work_start_hour}-{work_end_hour})..."
        )
    ...
    def generate_report(self) -> None:
        ...
        if self.events:
            logger.info(
                f"Time Range: {self.events[0]['timestamp_str']} to {self.events[-1]['timestamp_str']}"
            )
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("timeline_analysis"), help="Output directory"
    )
    parser.add_argument("--detect-anomalies", action="store_true", help="Detect activity anomalies")
    parser.add_argument(
        "--correlation-window", type=int, default=300, help="Event correlation window (seconds)"
    )
    parser.add_argument(
        "--spike-window", type=int, default=60, help="Activity spike detection window (minutes)"
    )
    parser.add_argument(
        "--gap-threshold", type=int, default=120, help="Timeline gap threshold (minutes)"
    )
