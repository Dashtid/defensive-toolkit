...
class TimelineGenerator:
    ...
    def check_plaso(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["log2timeline.py", "--version"], capture_output=True, timeout=5
            )
    ...
    def export_plaso_timeline(self, plaso_file: Path) -> bool:
        ...
        try:
            cmd = ["psort.py", "-o", "l2tcsv", "-w", str(self.output_file), str(plaso_file)]
    ...
    def generate_report(self, analysis: Dict) -> None:
        ...
        for day, count in sorted(
            analysis["daily_distribution"].items(), key=lambda x: x[1], reverse=True
        )[:5]:
